...
function downloadFile(url, destPath, redirectCount = 0) {
  return new Promise((resolve, reject) => {
    ...
    const req = https.get(
      ...
      {
        headers: {
          'User-Agent': '@claude-paper/cli postinstall',
...
async function maybeInstallBinary() {
  ...
  safeLog(`📦 Claude Paper: installing native binary for ${platformArch} (v${version})`)
  ...
  try {
    ...
    safeLog(`✅ Claude Paper: native binary ready at ${dest}`)
  } catch (err) {
    safeWarn(`⚠️  Claude Paper: could not download native binary (${platformArch})`)
    safeWarn(`    URL: ${url}`)
    safeWarn(
      `    Reason: ${err instanceof Error ? err.message : String(err)}`,
    )
    safeWarn(`    This is non-fatal. Claude Paper will fall back to Bun if available.`)
  }
}

async function postinstallNotice() {
  safeLog('✅ @claude-paper/cli installed. Commands available: cpaper, cp')
  safeLog('   If shell cannot find them, reload your terminal or reinstall globally:')
  safeLog('   npm i -g @claude-paper/cli  (or use: npx @claude-paper/cli)')
