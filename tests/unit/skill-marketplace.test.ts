...
describe('skill marketplace (local .kode-plugin/marketplace.json)', () => {
  ...
  test('add marketplace + install/uninstall skills into user dir', async () => {
    await withEnv(
      { KODE_CONFIG_DIR: join(homeDir, '.claude-paper') },
      async () => {
        ...
        expect(
          existsSync(join(homeDir, '.claude-paper', 'skills', 'pdf')),
        ).toBe(true)
        expect(
          existsSync(join(homeDir, '.claude-paper', 'skills', 'xlsx')),
        ).toBe(true)
  ...
  test('project install writes to ./.claude-paper/skills based on agent cwd', async () => {
    await withEnv(
      { KODE_CONFIG_DIR: join(homeDir, '.claude-paper') },
      async () => {
        ...
        expect(install.installedSkills.sort()).toEqual(['pdf', 'xlsx'])
        expect(
          existsSync(join(projectDir, '.claude-paper', 'skills', 'pdf')),
        ).toBe(true)
        expect(
          existsSync(join(projectDir, '.claude-paper', 'skills', 'xlsx')),
        ).toBe(true)
      },
    )
  })
          ...
          JSON.stringify(
            {
              ...
              owner: { name: 'Test', email: 'test@example.com' },
              metadata: {
                description: 'Updated marketplace',
                version: '1.0.0',
              },
              plugins: [
                {
