...
const PROJECT_URL = 'https://github.com/FredFang1216/MARs'
const DEFAULT_CO_AUTHOR = 'Claude Paper'
...
function getAttribution(): { commit: string; pr: string } {
  const pr = `🤖 Generated with [Claude Paper](${PROJECT_URL})`
  const commit = `${pr}\n\n   Co-Authored-By: ${DEFAULT_CO_AUTHOR} <noreply@claude-paper.ai>`
  return { commit, pr }
}
