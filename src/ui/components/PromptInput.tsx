...
function PromptInput({
  ...
  // Ref-based flag to suppress menu rendering during submission.
  // Refs update synchronously (unlike setState), so when Ink captures
  // the current frame for Static commitment, the menu is already hidden.
  const suppressMenuRef = React.useRef(false)
  ...
  // Reset the menu suppression flag once suggestions are actually cleared
  useEffect(() => {
  ...
  const renderedSuggestions = useMemo(() => {
    ...
    // Limit visible items to avoid exceeding terminal height.
    // Ink cannot clear lines that scroll above the viewport,
    // so a tall menu permanently "burns" into terminal scrollback.
    const MAX_VISIBLE = 10
    let startIndex = 0
    let endIndex = suggestions.length

    if (suggestions.length > MAX_VISIBLE) {
      // Sliding window centered on the selected item
    ...
    const items = suggestions
      ...
      .map((suggestion, i) => {
        ...
        return (
          <Box
            key={`${suggestion.type}-${suggestion.value}-${realIndex}`}
            flexDirection="row"
          >
      ...
      if (endIndex < suggestions.length) {
        items.push(
          <Box key="scroll-down" flexDirection="row">
            <Text dimColor> ↓ {suggestions.length - endIndex} more</Text>
          </Box>,
        )
      }
    }
  ...
  async function onSubmit(input: string, isSubmittingSlashCommand = false) {
    // If input is a slash command that matches a known command, treat as
    // slash command submission regardless of autocomplete menu state.
    // This fixes the bug where the menu guard blocks submission when the
    // user types a full command manually while autocomplete is open.
    ...
    if (
      ...
      commands.some(
        cmd =>
          trimmedInput === `/${cmd.userFacingName()}` ||
          trimmedInput.startsWith(`/${cmd.userFacingName()} `),
      )
    ) {
    ...
    // Synchronously suppress menu rendering via ref (instant, no re-render needed).
    // This ensures Ink won't capture the menu in the frame that gets committed to
    // Static/scrollback when the command output is added.
  ...
  return (
    <Box flexDirection="column">
      ...
      {!suppressMenuRef.current && suggestions.length > 0 && (
        <Box
          flexDirection="row"
          justifyContent="space-between"
          paddingX={2}
