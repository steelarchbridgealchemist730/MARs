...
export function useUnifiedCompletion({
  ...
  useInput((inputChar, key) => {
    if (
      ...
      // If the user's input already exactly matches a known slash command
      // (e.g., they typed "/propose" in full), do NOT replace the input
      // with the selected completion. Instead, just close the menu and
      // let the normal onSubmit handler process it as a command.
      const trimmed = input.trim()
      if (state.context?.type === 'command' && trimmed.startsWith('/')) {
        const typedCmd = trimmed.split(/\s+/)[0].slice(1) // e.g., "propose"
        const isExactCommand = commands.some(
          cmd =>
            cmd.userFacingName() === typedCmd ||
            (cmd.aliases && cmd.aliases.includes(typedCmd)),
        )
        if (isExactCommand) {
          resetCompletion()
          // Let the Enter event propagate to onSubmit naturally
          if (onSubmit) {
            onSubmit(input, true)
          }
          return true
  ...
  useEffect(() => {
    ...
    // During suppression window, still clear if input no longer has any
    // completion trigger (e.g. user deleted the "/" or "@" entirely).
    // This fixes the menu staying visible after backspace-deleting "/".
    if (Date.now() < state.suppressUntil) {
      if (state.isActive && !input.includes('/') && !input.includes('@')) {
        resetCompletion()
      }
      return
    }
    ...
    if (context && shouldAutoTrigger(context)) {
      ...
      // No completion context at all but menu is active — force clear.
      // This catches IME switches and other edge cases where the trigger
      // character disappears without going through the normal clear path.
      resetCompletion()
      return
