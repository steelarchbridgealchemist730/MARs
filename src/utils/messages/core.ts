...
export function getInProgressToolUseIDs(
  ...
  function isQueuedWaitingProgressMessage(message: NormalizedMessage): boolean {
    ...
    const inner = message.content?.message?.content
    if (!Array.isArray(inner)) return false
    const firstBlock = inner[0]
...
export function normalizeMessagesForAPI(
  ...
  // Defensive: inject synthetic tool_result for any orphaned tool_use blocks.
  // This prevents API 400 errors when a tool_use has no corresponding tool_result
  // (e.g., due to race conditions during user interrupts or aborted tool calls).
  ...
  for (let i = 0; i < result.length; i++) {
    ...
    for (const block of content) {
      ...
      if (allToolResultIds.has(block.id)) continue
      // Orphaned tool_use — inject a synthetic tool_result after this assistant message
      const syntheticResult = createUserMessage([
        {
          type: 'tool_result',
          tool_use_id: block.id,
          content: 'Tool execution was interrupted or did not return a result.',
          is_error: true,
        },
      ])
      // Insert right after the assistant message (or merge into existing user message)
      const nextIdx = i + 1
      if (nextIdx < result.length && result[nextIdx].type === 'user') {
        result[nextIdx] = mergeUserMessages(
          syntheticResult,
          result[nextIdx] as UserMessage,
        )
      } else {
        result.splice(nextIdx, 0, syntheticResult)
      }
      allToolResultIds.add(block.id)
    }
  }
