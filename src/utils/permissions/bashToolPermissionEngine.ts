...
function validatePathRestrictedCommand(
  ...
  if (hasCdInCompound && op !== 'read') {
    return {
      behavior: 'ask',
      message:
        "Commands that change directories and perform write operations require explicit approval to ensure paths are evaluated correctly. For security, Claude Paper cannot automatically determine the final working directory when 'cd' is used in compound commands.",
      decisionReason: {
        type: 'other',
        reason:
...
function validateOutputRedirections(
  ...
  if (hasCdInCompound && redirections.length > 0) {
    return {
      behavior: 'ask',
      message:
        "Commands that change directories and write via output redirection require explicit approval to ensure paths are evaluated correctly. For security, Claude Paper cannot automatically determine the final working directory when 'cd' is used in compound commands.",
      decisionReason: {
        type: 'other',
        reason:
